...
```
src/
├── SpanKind.php                  — enum: OTLP span kinds (Internal, Server, Client, Producer, Consumer)
├── SpanStatusCode.php            — enum: OTLP status codes (Unset, Ok, Error)
├── TraceId.php                   — generate / fromSeed / isValid for 32-hex trace IDs
├── SpanId.php                    — generate / isValid for 16-hex span IDs
├── SpanContext.php               — readonly value object: traceId, spanId, sampled
├── TraceParent.php               — parse/format the W3C `traceparent` header
├── Span.php                      — mutable span: attributes, start/end time, status; end() is idempotent
├── SpanExporterInterface.php     — contract: export(list<Span>): bool, must never throw
├── Tracer.php                    — startSpan() / endSpan(); exports each span when it ends
├── Otel.php                      — static facade around the Tracer singleton
├── OtelMiddleware.php            — SERVER span per HTTP request; continues `traceparent`
├── OtelServiceProvider.php       — binds Tracer, picks the exporter from config, wires the facade
└── Exporter/
    ├── SpanTransportInterface.php — the outbound HTTP POST behind OtlpHttpExporter
    ├── StreamSpanTransport.php    — default transport: PHP stream wrapper, no ext-curl
    ├── OtlpHttpExporter.php       — builds the OTLP/HTTP JSON ExportTraceServiceRequest and POSTs it
    ├── InMemorySpanExporter.php   — collects spans in memory (tests, local debugging)
    └── NullSpanExporter.php       — discards spans (default when no endpoint is configured)

tests/
├── TestCase.php
├── Support/                      — FakeConfig, FakeContainer (same stubs as sibling modules)
└── one *Test.php per class above (SpanExporterTest covers both simple exporters)
```
...
- **Tracer** — the only place spans are created and finished. `startSpan()` continues a real `SpanContext` parent, or a bare trace ID (no parent span), or starts a new trace. `endSpan()` ends the span and exports it immediately.
- **OtelMiddleware** — precedence for the trace ID: valid incoming `traceparent` header (real parent span) > seed resolver closure (trace ID only) > fresh ID. Span status is `Error` on 5xx responses and on exceptions (which are rethrown); 4xx stays `Ok`.
- **OtlpHttpExporter** — hand-builds the OTLP JSON mapping (`intValue`/`startTimeUnixNano` as strings, per the spec) and delegates the HTTP call to a `SpanTransportInterface`. Returns false instead of throwing on encode or transport failure.
- **OtelServiceProvider** — `otel.exporter` = `otlp` (default when `otel.endpoint` is set) / `memory` / anything else discards. Does not register the middleware; the application does.

---

## Design decisions and constraints

- **No vendor SDK, no protobuf.** Only the JSON encoding of OTLP/HTTP is implemented, hand-built. Adding another signal or encoding belongs in a follow-up, not a dependency.
- **Correlation with `ez-php/logging` is by seed, not by import.** This module does not depend on `ez-php/logging`; the application passes a closure returning the correlation ID, and `TraceId::fromSeed()` maps it to a 32-hex trace ID. The `request_id` in `RequestContextMiddleware` is generated inside that middleware and is not exposed on the request, so the closure must read it from wherever the application stores it.
- **Export is synchronous, one request per ended span.** Simple and explicit; latency is bounded by the transport timeout (5 s default). Buffering is an exporter concern.
- **Exporters never throw.** A tracing failure must not fail the request being traced.
- **The `Otel` facade holds static state.** Intentional, documented, and matches `Health`/`Log`; tests call `Otel::resetTracer()`.
- **Depends on `ez-php/contracts` and `ez-php/http` only.** `ez-php/framework` is a dev dependency (test of the provider uses local fakes) and a `suggest`.

---

## Testing approach

No external infrastructure. The OTLP exporter is tested against a recording `SpanTransportInterface` fake; `StreamSpanTransport` is only tested for its unreachable-endpoint failure path (no local HTTP server). Middleware tests use the real `EzPhp\Http\Request`/`Response`.

---

## What does not belong in this module

- **Metrics and logs signals** — Prometheus metrics live in `ez-php/metrics`; logging in `ez-php/logging`.
- **Auto-instrumentation** of the database, HTTP client, queue, etc. — those modules (or the application) create spans through `Otel::tracer()`.
- **Sampling, batching, and retry policies** — wrap `SpanExporterInterface`.
- **A collector or backend** — this module only exports.
